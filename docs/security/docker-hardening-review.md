...
2026-09-13. RustFS 1.0.0 upgrade evidence added 2026-09-17.
...
| RustFS image tag | `rustfs/rustfs:1.0.0-beta.2`. RustFS 1.0.0 GA was released 2026-09-16 (`rustfs/rustfs@sha256:8cc9801755448b71a786705ce76692c77e14936cccd87cf2fc31842e58f4d1ff`). **Upgrade attempted 2026-09-17 and reverted:** on the existing beta.2 `./data/rustfs` directory, 1.0.0 fails store init (`commit_pool_meta_identity` → `.rustfs.sys/pool.bin.identity`, `Bad file descriptor`, "pool metadata writes remain blocked pending fenced transaction recovery"), exhausts its retry budget, and restart-loops while S3 returns `503 x-rustfs-readiness-pending: storage_quorum`. On a fresh empty directory, 1.0.0 is ready in ~8 s. Rolling back to beta.2 restored the data (catalog rows intact). Environment: Docker Desktop on macOS, bind-mounted data dir. | Beta pin in a release; upgrade path for existing local data is unknown (format change vs. bind-mount bug not yet distinguished). The Compose healthcheck probes `/health` (liveness), which stayed `200` throughout the failure, so Compose reported the broken container as healthy. | Keep beta.2 until a migration path is confirmed (upstream release notes, or a wipe-and-reingest procedure accepted for local data). Switch the healthcheck to `/health/ready` (available on beta.2 and 1.0.0) so readiness failures are visible. |
| Lakekeeper image tag | `quay.io/lakekeeper/catalog:v0.13.5` (Compose; a stack started before the bump may still run v0.13.3 until recreated) | Release-tag drift over time | Consider digest pinning before release-grade publishing |
