...
2026-09-13. RustFS 1.0.0 upgrade and named-volume evidence added 2026-09-17.
...
| RustFS image tag and storage | `rustfs/rustfs:1.0.0` (GA 2026-09-16, `rustfs/rustfs@sha256:8cc9801755448b71a786705ce76692c77e14936cccd87cf2fc31842e58f4d1ff`), data in the named volume `rustfs-data`. **Upgrade evidence (2026-09-17, Docker Desktop on macOS):** 1.0.0 on the beta.2 bind-mounted `./data/rustfs` failed store init (`commit_pool_meta_identity`, `Bad file descriptor`, retry budget exhausted, restart loop) while `/health` stayed `200`; on a fresh bind mount it started but bucket creation returned `500 InternalError` with the same EBADF on rename, and it logged `Unsupported filesystem type detected ... /data (FUSE)`. On a named volume: bucket/object PUT/GET `200`, no warning, then `init` → Docker-gated `ingest_then_query_flow` → real `ingest` → re-ingest (`already_exists`) → `query` all passed. | Beta→GA has no in-place migration; local lakehouse contents must be re-ingested (manual documents the reset). RustFS declares bind mounts unsupported, so operators who re-add one on macOS will hit write failures. | Digest pin before release-grade publishing. Readiness healthcheck (`/health/ready`) now in place so a non-ready store is visible. |
| Lakekeeper image tag | `quay.io/lakekeeper/catalog:v0.13.5` | Release-tag drift over time | Consider digest pinning before release-grade publishing |
...
| Runtime container smoke test | S5-C local image build and `docker run --rm anti_entropator:local-scan --help` passed on 2026-05-27 with the then-current `rust:1.95-bookworm` builder and `debian:bookworm-slim` runtime. The `rust:1.96-bookworm` builder (#107, 2026-06-04) was exercised runner-side by release workflow_dispatch run `27274110998` (2026-06-10), which also passed smoke + Trivy in the release container gate on canonical tag `ghcr.io/sm4rtm4art/anti_entropator:sha-bc5ebf2`. First guarded tag-push evidence is now captured by release run `28154844011` (2026-06-25) on `v0.3.0-rc.1`: `publish-container` completed smoke + Trivy then pushed `ghcr.io/sm4rtm4art/anti_entropator:0.3.0-rc.1` and `:sha-29a4fec` at digest `sha256:5b7d674c34d3cc52cdc8acdb7bd1ef96fde28203062b4ae4c04ece91b3cf431b`, followed by successful `create-release`. This was a throwaway rehearsal: the `v0.3.0-rc.1` tag, GitHub pre-release, and GHCR image versions (`0.3.0-rc.1`, `sha-29a4fec`) were removed after evidence capture; run `28154844011` remains the durable evidence. Since 2026-09-20 the release-path `container-verify` (both `verify-container` and `publish-container`) additionally runs the application smoke `init` -> `ingest` -> `query` with the built image against an ephemeral green Compose slot (`app-smoke: "true"`, `scripts/delivery-sim.sh`) before Trivy; `security.yml` keeps the `--help` smoke only. First run evidence is due with the `v0.3.1` tag. | Low for release-tag publish path evidence; remaining gap is that main-branch CI runtime-image publishing is still a separate non-gated path | Keep smoke check before Trivy scans in `security.yml`; preserve this run as baseline evidence and continue tracking main-branch CI image-scan gating as a separate follow-up |
...
| Release container re-runs | The release container path no longer uses `verified-image-*`/`publish-image-*` tarball handoff artifacts. Each container job builds and verifies the canonical image once in-job (`verify-container` on dispatch, `publish-container` on tag push); `publish-container` then pushes that exact verified image without an intervening rebuild. | Re-run failed jobs no longer reuse stale handoff artifacts, but re-runs can take longer because build+scan are repeated. | Keep this behavior for determinism and reduced plumbing; revisit only if repeated release re-runs show unacceptable latency. |
| Runner disk headroom and cargo cache size | Free-disk-space handling is now centralized via `.github/actions/free-disk-space` and reused in quality, coverage, release-binaries, delivery-smoke, and multi-arch dry-run jobs. `setup-rust` now restores only suffix-scoped caches (broad `Linux-cargo-` fallback removed), and Cargo Audit disables cache restore/save (`cache-enabled: "false"`) because it does not compile project targets. `target` remains cached for lint/test/release jobs that benefit from incremental builds. | Hosted runners still provision ~14 GB free, so cold runs can remain slower and large per-suffix `target` caches still consume cache storage. | Keep per-suffix cache scoping and audit cache-off defaults; monitor runtime and disk trends in weekly/security/release evidence and tune cache scopes only if new pressure appears. |
| macOS release-build duration | Measured `Build aarch64-apple-darwin` job durations from release `workflow_dispatch` history vary by cache warmth (not the fixed "~38 min" previously claimed): cold runs `27274110998` and `27302097110` (2026-06-10) took ~43 min each, while warmer runs `27376267218` (~22 min), `27413340130` (~27 min), and `27427259430` (~21 min) were materially faster. The leg is now pinned to `macos-15` (#131), so future durations are measured against a fixed runner image. S5-D Slice B3 root-cause: the warm floor is set by `[profile.release]` `lto = true` (fat LTO) + `codegen-units = 1`, which serialize codegen and redo whole-program optimization of the heavy DataFusion/Arrow/Parquet/Iceberg final crate (not cache-skippable). Per-suffix `~/.cargo/registry` + `~/.cargo/git` + `target` caching (B2) is effective and accounts for the ~16-22 min cold-vs-warm gap by skipping unchanged dependency recompiles. | The leg is the slowest release step (~21-27 min warm, ~43 min cold). Reducing the warm floor means trading release-binary size/runtime quality for build speed. | Decision (S5-D, deferred): accept the current fat-LTO release profile (smaller, faster published binary is the intended trade; warm duration is acceptable). Bounded optimization candidate, gated on explicit approval because it changes release-artifact characteristics: `lto = "thin"` and/or higher `codegen-units` (optionally a release-only override), re-measuring size and duration before adopting. |
