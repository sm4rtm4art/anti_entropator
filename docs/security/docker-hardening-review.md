...
| macOS release-build duration | `Build aarch64-apple-darwin` in release workflow dispatch runs is consistently high (about 38 minutes in S5-C Slice D validation runs). | Long macOS leg slows reruns and closeout iteration, even when Linux jobs are green. | Track as S5-D follow-up: evaluate cache effectiveness and candidate optimizations for the macOS release build leg. |
