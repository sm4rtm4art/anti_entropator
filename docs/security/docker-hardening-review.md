...
| Runner disk headroom and cargo cache size | Free-disk-space steps run in quality test/audit, release `build-binaries`, and the multi-arch dry-run. S5-D Slice B2 removed the broad `Linux-cargo-` fallback restore key from `setup-rust` so each job only restores its own suffix-scoped cache (no cross-job multi-GB bleed), and disabled cache restore/save entirely for the Cargo Audit job (`cache-enabled: "false"`), which compiles nothing. `target` is still cached for the lint/test/release jobs that benefit from incremental builds, but only via the per-suffix key. | Hosted runners still provision ~14 GB free; release/macOS and multi-arch jobs can still be slow, and per-suffix `target` caches still consume cache storage. | Keep per-suffix cache scoping and the audit cache-off default; monitor post-change runner disk headroom and execution duration. Evaluate any remaining latency in the dedicated macOS follow-up slice. |
