...
| macOS release-build duration | `Build aarch64-apple-darwin` in release workflow dispatch runs is consistently high (about 38 minutes in S5-C Slice D validation runs). S5-D Slice B3 root-caused this from config rather than fresh CI measurement: `[profile.release]` uses `lto = true` (fat LTO) and `codegen-units = 1`, which serialize codegen and redo whole-program optimization of the heavy DataFusion/Arrow/Parquet/Iceberg graph on every source change. The leg already caches `~/.cargo/registry` + `~/.cargo/git` + `target` per `release-binaries-aarch64-apple-darwin` suffix (B2), but that cache only avoids recompiling unchanged dependency objects on unchanged-`Cargo.lock` re-runs; the final-crate fat-LTO link is not cache-skippable and dominates the wall time. | Long macOS leg slows reruns and closeout iteration, even when Linux jobs are green. Reducing it means trading release-binary size/runtime quality for build speed. | Decision: accept the current fat-LTO release profile for now (smaller, faster published binary is the intended release-quality trade). Bounded optimization candidate, gated on explicit approval because it changes release-artifact characteristics: switch to `lto = "thin"` and/or raise `codegen-units` (optionally via a release-only profile override) and re-measure size and duration before adopting. |
